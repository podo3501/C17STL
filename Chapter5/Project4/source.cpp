...
int main()
{
	...
	auto found_cologne1(find(begin(c), end(c), city{ "Cologne", 1060000 }));
	print_city(found_cologne1);

	auto found_cologne2(find_if(begin(c), end(c), [](const auto& item) { return item.name == "Cologne"; }));
	print_city(found_cologne2);
